...
class search {
  ...
  private function connect() {
    $this->mysqli = new mysqli( 'localhost', 'xfmkwpty_app921', 'fTLe$,e=}T^e', 'xfmkwpty_app921' );
  }
  ...
  public function search($search_term) {
    ...
    $query = $this->mysqli->query("
      SELECT broker, country, city, state
      FROM listings_table
      WHERE  broker LIKE '%{$sanitized}%'
      OR country LIKE '%{$sanitized}%'OR state LIKE '%{$sanitized}%'
    ");
